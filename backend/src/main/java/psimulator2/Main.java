    ...
    public static void main(String[] args) {
        ...
        if (args.length >= 2) {
            try {
                ...
                Logger.log(Logger.WARNING, LoggingCategory.XML_LOAD_SAVE,
                        "Second argument " + args[1] + " is not port number. Using default value: " + firstTelnetPort);
            }
        }
        ...
        if (args.length >= 3) {
            try {
                ...
                Logger.log(Logger.WARNING, LoggingCategory.XML_LOAD_SAVE,
                        "Third argument " + args[2] + " is not port number. Using default value: " + eventServerPort);
            }
        }
        ...
        try {
            networkModel = serializer.loadNetworkModelFromFile(new File(
                    configFileName));
        } catch (SaveLoadException ex) {
            Logger.log(Logger.WARNING, LoggingCategory.XML_LOAD_SAVE, Util.
                    stackToString(ex));
            Logger.log(Logger.ERROR, LoggingCategory.XML_LOAD_SAVE,
        ...
        Loader loader = new Loader(networkModel, configFileName);
        loader.loadFromModel();	// simulator se startuje z tech ukladacich struktur

        // startovani telnetu:
        TelnetD telnetDaemon;
        Logger.log(Logger.INFO, LoggingCategory.TELNET,
                "Starting telnet listeners");
        try {

            telnetDaemon = TelnetD.createTelnetD(TelnetProperties.
                    getProperties());
            // @TODO pridat metodu na kontrolu obsazení portů
            telnetDaemon.start();

            Logger.log(Logger.INFO, LoggingCategory.TELNET,
                    "Telnet listeners successfully started");

        } catch (BootException ex) {
            Logger.log(Logger.DEBUG, LoggingCategory.TELNET, ex.toString());
            Logger.log(Logger.ERROR, LoggingCategory.TELNET,
                    "Error occured when creating telnet servers.");
        }
        ...
        while (true) {
            ...
            Logger.log(Logger.WARNING, LoggingCategory.EVENTS_SERVER,
                    "Port " + eventServerPort + " not available, using:" + (eventServerPort + 1));

            tryMark++;
            eventServerPort++;

            if (tryMark > maxTryMark) {
                Logger.log(Logger.ERROR, LoggingCategory.EVENTS_SERVER,
                        "Cannot start event server, no port available");
            }
        }
        ...
        Logger.log("PACKET FLOW SERVER: ", Logger.IMPORTANT,
                LoggingCategory.EVENTS_SERVER,
                "Server sucessfully started, listening on port: " + eventServerPort);
